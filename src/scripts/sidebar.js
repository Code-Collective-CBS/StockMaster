...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setActiveSidebarItem() {
        ...
        if (currentPath.includes('dashboard') || currentPath === '/' || currentPath.endsWith('index.html')) {
            // Dashboard/home page
        ...
        } else if (currentPath.includes('stocks') || currentPath.includes('aktier')) {
            // Stocks page
            document.querySelector('.nav-item:nth-child(3)').classList.add('active');

        } else if (currentPath.includes('settings') || currentPath.includes('indstillinger')) {
            // Settings page - note this is in the bottom nav
            document.querySelector('.setting-item:nth-child(1)').classList.add('active');
        }
    }
    ...
    const logoutButton = document.getElementById('logoutButton') || document.querySelector('.signOut');
    if (logoutButton) {
    ...
    sidebarLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            // page transition could we add here looks good
        });
    });
});