...
const getUserInfo = async () => {
  try {
      const response = await fetch('http://localhost:3000/api/database/getUser')
      if (response.ok) {
          const data = await response.json()
          document.getElementById('profile-name').textContent = `${data.fornavn} ${data.efternavn}`
          console.log(`${data.fornavn} ${data.efternavn}`)
      } else {
          document.getElementById('profile-name').textContent = 'Hej'
          console.log(data)
      }
  } catch(err) {
