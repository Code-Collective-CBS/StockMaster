export const stockMetrics = {
  ...
  populateDividends: function (companyData) {
    ...
    if (!companyData.DividendPerShare || companyData.DividendPerShare === "0") {
      const dividendSection = document.querySelector(
        ".stock-info-section:nth-child(2)"
      );
      if (dividendSection) {
  ...
  createAnalystRatingsChart: function (companyData) {
    console.log("Creating analyst ratings chart");
    ...
    // Ødelæg tidligere chart hvis det eksisterer
    if (this.chartInstances.analystRatings) {
      this.chartInstances.analystRatings.destroy();
      this.chartInstances.analystRatings = null;
    }
    ...
    try {
      // Create the chart
      this.chartInstances.analystRatings = new Chart(canvas, {
        type: "bar",
        data: {
      ...
      return this.chartInstances.analystRatings;
    } catch (error) {
